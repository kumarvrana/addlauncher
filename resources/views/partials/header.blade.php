  ...
  <div class="container-fluid">
    <div class="row">
      <div class="visible-sm visible-xs col-sm-2 col-xs-3">
       <i class="fa fa-bars toggle_menu visible-sm visible-xs wow fadeInLeft""></i>
    ...
    <div class="col-lg-4 col-md-3 hidden-sm hidden-xs">
        ...
        <div class="cd-dropdown-wrapper">
          ...
          <nav class="cd-dropdown">
            ...
            <ul class="cd-dropdown-content">
              ...
              @if($mediacat->label=='Airport')
              <li class="has-children"><a href="{{env('APP_URL')}}{{$mediacat->slug}}"><span class="cat-menu-img {{str_replace(' ','_', strtolower($mediacat->title))}} "></span>&nbsp;{{$mediacat->label}}</a>
                <ul class="cd-dropdown-icons is-hidden">

                  <li class="go-back"><a href="#0">Menu</a></li>
                  @foreach($airport_menu as $key => $value)
                  <li> <a href="{{ route('frontend.getfrontAirportadByOption', ['airportOption' => $key]) }}"  class="cd-dropdown-item item-{{$loop->iteration}}">
                    <h3>{{$value}}</h3>
              ...
              <li class="has-children"><a href="{{env('APP_URL')}}{{$mediacat->slug}}"><span class="cat-menu-img {{str_replace(' ','_', strtolower($mediacat->title))}} "></span>&nbsp;{{$mediacat->label}}</a>
                <ul class="cd-dropdown-icons is-hidden">
                  @foreach($auto_menu as $key => $value)
                  <li><a href="{{ route('frontend.getfrontAutoadByType', ['autotype' => $key]) }}" class="cd-dropdown-item item-{{$loop->iteration}}"><h3>{{$value}}</h3>
                    <p>This is the item description</p></a></li>
                    @endforeach
                    <div class="img_mega">
                      <img src="{{asset('images/'.$mediacat->image)}}" class="img-responsive">
                    </div>
                ...
                <li class="has-children"><a href="{{env('APP_URL')}}{{$mediacat->slug}}"><span class="cat-menu-img {{str_replace(' ','_', strtolower($mediacat->title))}} "></span>&nbsp;{{$mediacat->label}}</a>
                  <ul class="cd-dropdown-icons is-hidden">
                    @foreach($car_menu as $key => $value)
                    <li><a href="{{ route('frontend.getfrontCaradByType', ['cartype' => $key]) }}" class="cd-dropdown-item item-{{$loop->iteration}}"><h3>{{$value}}</h3>
                      <p>This is the item description</p></a></li>
                      @endforeach
                      <div class="img_mega">
                      <img src="{{asset('images/'.$mediacat->image)}}" class="img-responsive">
                  ...
                  <li class="has-children"><a href="{{env('APP_URL')}}{{$mediacat->slug}}"><span class="cat-menu-img {{str_replace(' ','_', strtolower($mediacat->title))}} "></span>&nbsp;{{$mediacat->label}}</a>
                    <ul class="cd-dropdown-icons is-hidden">
                      @foreach($outdoor_menu as $key => $value)
                      <li><a href="{{ route('frontend.getfrontBillboardadByOption', ['billboardOption' => $key]) }}" class="cd-dropdown-item item-{{$loop->iteration}}"><h3>{{$value}}</h3>
                        <p>This is the item description</p></a></li>
                        @endforeach
                        <div class="img_mega">
                      <img src="{{asset('images/'.$mediacat->image)}}" class="img-responsive">
                    ...
                    @else
                    <li><a href="{{env('APP_URL')}}{{$mediacat->slug}}"><span class="cat-menu-img {{str_replace(' ','_', strtolower($mediacat->title))}} "></span>&nbsp;{{$mediacat->label}}</a></li>
                    @endif
                ...
                <div class="drop_inner_login">
                  ...
                  <div class="sub_log">
                    <div class="forgot_pw">
                      ...
                      <div class="btn thb-fill-style">
                        <span><form action="{{ route('user.postsignout') }}" method="POST" id="logout-form">{{csrf_field()}}<a href="#" onclick="document.getElementById('logout-form').submit()">Logout</a></form></span>
                      </div>
                    </div>
                  </div>
      ...
      <div class="col-lg-2 col-md-3 hidden-sm hidden-xs">
        <div class="head_support pull-right">
          ...
          <!-- <p>[Call Now]</p> -->
          <h5><img src="http://parasfashion.co.uk/icons/WhatsApp_Icon.png"> <span>+91-9888575401</span></h5>
        </div>
      </div>
    </div>

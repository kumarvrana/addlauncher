...
@section('content')
        ...
        <div class="container-fluid"> <!-- container fluid 1 starts here -->
            <div class="row">
                ...
                @PHP
                   $variations = unserialize($metroad->display_options);
				   
				   $variation = array();
                   $price = array();
				   $passVariation = array();
                  foreach($metroprice as $options){
			     		...
			     		@foreach($variations as $loop)
			     			<style type="text/css">
			     				.{{strtolower(str_replace(' ','_', $name[$i]))}}{
									background-image: url('../../images/display/metro/{{strtolower(str_replace(' ','_', $name[$i]))}}.png');
								} 
							</style>
			     			<div class="col-md-3 col-sm-3 "> 
				     			<div class="pro-item"> 
								    ...
								    <p class="font-3">{{$number[$i]}} Metro for {{$duration[$i]}} months</p>
								    <p class="font-2"><del class="lighter">Rs {{$new_price[$i]}}</del>Rs {{$new_price[$i]}}</p>
								      @PHP
									$options = $new_price[$i].'+'.$passVariationname[$i];
									$session_key = 'metros'.'_'.$passVariationname[$i].'_'.$metroad->id;
									$printsession = (array) Session::get('cart');
													
									@ENDPHP
								    <div class="clearfix"> 
								    	<a class="glass" href="{{route('metro.addtocart', ['id' => $metroad->id, 'variation' => $options])}}"><span class="fa fa-star"></span>
